...
==============================================================================
 ADAPTAR A TU IOLClient
==============================================================================
Interfaz esperada (confirmada contra el Swagger real de IOL):
...
if __name__ == "__main__":
    ...
    # ADAPTAR: inicialización real de tu IOLClient con credenciales desde
    # variables de entorno (secrets de GitHub).
    # iol_client = IOLClient(usuario=os.environ["IOL_USUARIO"],
    #                         password=os.environ["IOL_PASSWORD"])
    iol_client = None
    if iol_client is None:
        raise RuntimeError(
            "Falta inicializar iol_client con tu clase real. "
            "Ver sección 'ADAPTAR A TU IOLClient' al inicio de este archivo."
        )
