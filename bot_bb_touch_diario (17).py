...
estrategia. Resumen operativo de las 4 rutinas:
  ...
  2. rutina_monitoreo_fase_a()  -> cada 10 min, SOLO tickers en Fase A
     Vende a mercado si el precio en vivo toca el SL 15% (SL_INICIAL_PCT).
     Fase B no
...
# *** SALIDA HÍBRIDA DE FASE B (06/08/2026) *** -- ver rutina_cierre()
# para la lógica completa y la bitácora del chat del proyecto para el
# proceso de validación (backtest sobre 102 tickers: 730.5% retorno,
# 4.6% max drawdown, mejor que el baseline de "sale con 1 sola vela
# bajo EMA50" en las métricas principales).
UMBRAL_CORRIO_LEJOS_PCT = 15.0     # % sobre la EMA50 a partir del cual sale con 1 sola vela
UMBRAL_RUPTURA_FUERTE_PCT = 2.0    # % bajo la EMA50 que se considera "ruptura fuerte" (sale ya)
VELAS_CONFIRMACION_SALIDA = 2      # velas consecutivas de ruptura LEVE para confirmar la salida

# *** MODO EMA10 (10/08/2026) *** -- ver rutina_cierre() para la lógica
# completa. Validado en backtest: retorno 771.7%->812.5%, max drawdown
# 4.4%->4.2%, robusto ante concentración (81.0% del resultado sin el
# mejor trade). Se probó también con EMA20 en vez de EMA10 -- resultado
# prácticamente idéntico (810.5% retorno), se optó por EMA10 por ser
# conceptualmente más coherente (más sensible, reacciona más rápido a
# un trade que ya demostró ser excepcional). Ver bitácora del chat del
# proyecto, sesión del 10/08/2026, para el detalle completo.
UMBRAL_CORRIO_LEJOS_EMA10_PCT = 60.0   # % sobre la EMA50 a partir del cual se activa el modo EMA10
DISTANCIA_SALIDA_EMA10_PCT = 10.0      # % bajo la EMA10 que dispara la salida, una vez activado el modo

RUTA_TICKERS_CSV = "tickers_activos.csv"
CAPITAL_INICIAL_CUENTA = 100_000.0


def con_lock_rutina(nombre_rutina: str):
    """
    *** LOCK CONTRA CORRIDAS SUPERPUESTAS (19/08/2026) ***
    Decorador para las 4 rutinas -- ver docstring completo de
    `sheets.intentar_tomar_lock()` para el bug real que lo motivó
    (19/08/2026: rutina_cierre corrió dos veces casi al mismo tiempo --
    el respaldo nativo de GitHub coincidió con el disparo real de
    cron-job.org dentro de la misma ventana -- y la segunda pisó lo que
    había escrito la primera, borrando una posición real recién
    comprada de "Operaciones Activas").

    Se conecta a Sheets UNA VEZ acá (conexión extra, aparte de la que
    cada rutina hace igual como su primera línea -- redundante pero
    inofensivo, es solo una llamada más de autenticación) para poder
    tomar el lock ANTES de que la rutina haga cualquier otra cosa. Si
    no se consigue el lock (otra corrida en curso), la función decorada
    ni siquiera se llama. Si se consigue, se libera SIEMPRE al salir --
    haya terminado bien, con un return temprano, o con una excepción --
    gracias al try/finally.
    """
    def decorador(func):
        def wrapper(iol_client):
            sheet_lock = sheets.conectar_sheet()
            if not sheets.intentar_tomar_lock(sheet_lock, nombre_rutina):
                return
            try:
                return func(iol_client)
            finally:
                sheets.liberar_lock(sheet_lock)
...
def leer_niveles_guardados(sheet) -> dict:
    ...
    for r in registros:
        ...
        niveles[ticker] = {
            ...
            # *** SALIDA HÍBRIDA DE FASE B (06/08/2026) *** -- estado
            # persistido entre corridas, ver rutina_cierre() para la
            # lógica completa. "Superó 15%" queda marcado para siempre
            # una vez True (no se resetea); "Contador Ruptura Leve" se
            # resetea a 0 cada vez que cierra por encima de la EMA50.
            "supero_umbral_lejos": str(r.get("Superó 15%", "")).strip().upper() == "SI",
            "contador_ruptura_leve": int(r.get("Contador Ruptura Leve", 0) or 0),
            # *** MODO EMA10 (10/08/2026) *** -- también queda marcado
            # para siempre una vez True, mismo criterio que "Superó 15%".
            "modo_ema10": str(r.get("Modo EMA10", "")).strip().upper() == "SI",
...
def _registrar_compra_confirmada(sheet, ticker: str, tipo: str, precio_ejecutado: float,
                                  acciones: int, hoy: date) -> dict:
    """
    *** COMPARTIDO ENTRE COMPRAS NORMALES Y ÓRDENES MANUALES (16/08/2026) ***
    Registra una compra ya confirmada (precio real conocido) sobre el
    diccionario de niveles leído fresco de 'Operaciones Activas'.

    Si el ticker NO tenía posición abierta: la crea en Fase A con
    SL al 15% del precio ejecutado -- mismo comportamiento de siempre.

    Si el ticker YA tenía posición abierta (nunca pasa en las compras
    normales del bot, que siempre saltean tickers con posición -- pero
    SÍ puede pasar con una compra manual sobre un ticker que ya
    tenías): la COMBINA en una sola fila -- suma acciones, promedia el
    precio de entrada ponderado por cantidad, conserva la fecha de
    entrada más antigua. Si la posición combinada sigue en Fase A, el
    SL Fase A (y el Stop Vigente, que en Fase A es el mismo valor) se
    recalculan al 15% del nuevo precio promedio. Si ya está en Fase B,
    el Stop Vigente -- atado a la EMA50, no al precio de entrada -- NO
    se toca; solo cambian acciones y precio promedio (el SL Fase A
    combinado queda de referencia, aunque en Fase B no se usa para
    decidir la salida).

    Devuelve el diccionario de niveles ya actualizado -- no lo escribe
    en Sheets, eso lo hace el llamador con _reescribir_operaciones_activas
    (para no leer/escribir la hoja dos veces en el mismo flujo).
    """
    niveles = leer_niveles_guardados(sheet)
    if ticker in niveles:
        previo = niveles[ticker]
        acciones_totales = previo["acciones"] + acciones
        precio_promedio = (
            previo["precio_entrada"] * previo["acciones"] + precio_ejecutado * acciones
        ) / acciones_totales
        sl_promedio = precio_promedio * (1 - SL_INICIAL_PCT)
        niveles[ticker] = {
            **previo,
            "precio_entrada": precio_promedio,
            "acciones": acciones_totales,
            "sl_fase_a": sl_promedio,
            "stop_vigente": sl_promedio if previo["fase"] == "A" else previo["stop_vigente"],
        }
    else:
        sl_fase_a = precio_ejecutado * (1 - SL_INICIAL_PCT)
        niveles[ticker] = {
            "tipo": tipo, "fecha_entrada": hoy.strftime("%Y-%m-%d"),
            "precio_entrada": precio_ejecutado, "acciones": acciones,
            "fase": "A", "stop_vigente": sl_fase_a, "sl_fase_a": sl_fase_a,
        }
    return niveles


def _ejecutar_venta_manual(iol_client, sheet, ticker: str, nivel: dict, cantidad: int,
                            motivo: str, orden_manual_fila: int = None) -> dict:
    """
    *** ÓRDENES MANUALES (16/08/2026) *** -- variante de `_cerrar_posicion`
    que admite vender PARCIAL (no siempre todas las acciones de la
    posición). No toca `niveles` ni "Operaciones Activas" -- eso lo
    hace el llamador según el resultado, para poder decidir entre
    borrar la fila (venta total) o reducir "acciones" (parcial).

    Devuelve {"resultado": "cerrada"|"parcial"|"pendiente"|"error",
    "acciones_vendidas": int, "precio_salida": float|None, "error": str|None}.
    """
    try:
        orden_venta = iol_client.vender_mercado(ticker, cantidad)
    except Exception as e:
        return {"resultado": "error", "acciones_vendidas": 0, "precio_salida": None, "error": str(e)}

    if orden_venta.get("pendiente"):
        sheets.registrar_orden_pendiente(
            sheet, ticker, "venta", orden_venta.get("numero_operacion"),
            nivel.get("tipo", ""), cantidad, orden_venta.get("precio_estimado", 0),
            datos_extra={
                ...
                "orden_manual_fila": orden_manual_fila,
            },
        )
        tg.notificar_orden_pendiente(ticker, "venta")
        return {"resultado": "pendiente", "acciones_vendidas": 0, "precio_salida": None, "error": None}

    if not orden_venta.get("exito"):
        return {"resultado": "error", "acciones_vendidas": 0, "precio_salida": None,
                "error": f"orden rechazada: {orden_venta}"}

    precio_salida = orden_venta["precio_ejecutado"]
    hoy = hoy_argentina()
    desglose = calcular_pnl_con_desglose(nivel["precio_entrada"], precio_salida, cantidad)
    ...
    sheets.registrar_operacion_cerrada(
        ...
        hoy.strftime("%Y-%m-%d"), precio_salida, cantidad, nivel.get("sl_fase_a", 0),
        motivo, desglose["comision_total"], desglose["ddm_total"],
        desglose["pnl_pesos"], desglose["pnl_pct"], dias_holding,
    )
    tg.notificar_cierre_posicion(ticker, hoy.strftime("%d/%m/%Y"), precio_salida, motivo,
                                  desglose["pnl_pesos"], desglose["pnl_pct"])
    resultado = "cerrada" if cantidad >= nivel["acciones"] else "parcial"
    return {"resultado": resultado, "acciones_vendidas": cantidad, "precio_salida": precio_salida, "error": None}


def _procesar_compra_manual(iol_client, sheet, ticker: str, tipo: str, cantidad_pedida: int, fila: int):
    """Ver docstring de `procesar_ordenes_manuales` para el flujo
    completo. Ignora TOPE_MAXIMO_POSICION a pedido explícito ("sin
    límites, bajo mi responsabilidad") pero SIEMPRE respeta el efectivo
    real disponible -- compra lo que alcance, no todo o nada."""
    print(f"[ordenes_manuales] {ticker}: procesando COMPRA de {cantidad_pedida} acciones (fila {fila})")
    try:
        efectivo = iol_client.consultar_saldo()
    except Exception as e:
        print(f"[ordenes_manuales] {ticker}: error al consultar saldo -- {e}")
        sheets.actualizar_orden_manual(sheet, fila, "error", f"no se pudo consultar saldo: {e}")
        return
    if efectivo <= 0:
        print(f"[ordenes_manuales] {ticker}: sin capital disponible (efectivo=${efectivo:,.2f})")
        sheets.actualizar_orden_manual(sheet, fila, "error", "sin capital disponible")
        tg.notificar_error(f"compra manual {ticker}", "sin capital disponible")
        return

    try:
        precio_en_vivo = iol_client.obtener_precio(ticker)
    except Exception as e:
        print(f"[ordenes_manuales] {ticker}: error al obtener precio -- {e}")
        sheets.actualizar_orden_manual(sheet, fila, "error", f"no se pudo obtener precio: {e}")
        return

    # Sin tope de $100.000 -- se le pasa el propio efectivo como "tope"
    # a la misma función de dimensionamiento que usa el resto del bot,
    # así igual queda protegida por su red de seguridad de redondeo.
    # *** COLCHÓN DE PRECIO (21/08/2026) *** -- bug real: TGNO4.BA se
    # dimensionó justo al límite del efectivo disponible, pero el
    # precio subió un poco entre el momento en que el bot lo consultó
    # y el momento en que IOL evaluó la orden real -- el monto final
    # terminó ~$474 por encima de lo calculado, e IOL rechazó la orden
    # COMPLETA (no la recorta, la rechaza entera). Se deja un colchón
    # del 1% del efectivo como margen para absorber ese movimiento.
    MARGEN_SEGURIDAD_PRECIO_PCT = 0.01
    acciones_maximas = calcular_acciones_por_capital_objetivo(
        efectivo, precio_en_vivo, tope=efectivo * (1 - MARGEN_SEGURIDAD_PRECIO_PCT))
    acciones = min(cantidad_pedida, acciones_maximas)
    print(f"[ordenes_manuales] {ticker}: efectivo=${efectivo:,.2f} precio=${precio_en_vivo:,.2f} "
          f"-> acciones a comprar={acciones} (pedidas {cantidad_pedida})")
    if acciones <= 0:
        sheets.actualizar_orden_manual(
            sheet, fila, "error",
            f"capital insuficiente (efectivo ${efectivo:,.2f}, precio ${precio_en_vivo:,.2f})")
        tg.notificar_error(f"compra manual {ticker}",
                            f"capital insuficiente para ni 1 acción (efectivo ${efectivo:,.2f})")
        return
    nota_cantidad = "" if acciones == cantidad_pedida else \
        f" (se pidieron {cantidad_pedida}, solo alcanzó para {acciones})"

    try:
        orden_compra = iol_client.comprar_mercado(ticker, acciones)
    except Exception as e:
        print(f"[ordenes_manuales] {ticker}: error al enviar orden -- {e}")
        sheets.actualizar_orden_manual(sheet, fila, "error", f"error al enviar orden: {e}")
        tg.notificar_error(f"compra manual {ticker}", str(e))
        return

    if orden_compra.get("pendiente"):
        print(f"[ordenes_manuales] {ticker}: orden enviada, queda PENDIENTE de confirmar")
        sheets.registrar_orden_pendiente(
            sheet, ticker, "compra", orden_compra.get("numero_operacion"),
            tipo, acciones, orden_compra.get("precio_estimado", 0),
            datos_extra={"orden_manual_fila": fila, "orden_manual_nota": nota_cantidad},
        )
        sheets.actualizar_orden_manual(
            sheet, fila, "en curso", f"orden enviada ({acciones} acciones), esperando confirmación{nota_cantidad}")
        tg.notificar_orden_pendiente(ticker, "compra")
        return

    if not orden_compra.get("exito"):
        print(f"[ordenes_manuales] {ticker}: orden RECHAZADA -- {orden_compra}")
        sheets.actualizar_orden_manual(sheet, fila, "error", f"orden rechazada: {orden_compra}")
        tg.notificar_error(f"compra manual {ticker}", f"orden rechazada: {orden_compra}")
        return

    precio_ejecutado = orden_compra["precio_ejecutado"]
    print(f"[ordenes_manuales] {ticker}: COMPRA CONFIRMADA -- {acciones} acciones a ${precio_ejecutado:,.2f}")
    hoy = hoy_argentina()
    niveles = _registrar_compra_confirmada(sheet, ticker, tipo, precio_ejecutado, acciones, hoy)
    _reescribir_operaciones_activas(iol_client, sheet, niveles)
    sheets.actualizar_orden_manual(
        sheet, fila, "ejecutada", f"compradas {acciones} a ${precio_ejecutado:,.2f}{nota_cantidad}")
    tg.notificar_apertura_posicion(ticker, hoy.strftime("%d/%m/%Y"), precio_ejecutado,
                                    niveles[ticker]["sl_fase_a"], acciones=acciones)


def _procesar_venta_manual(iol_client, sheet, ticker: str, cantidad_pedida: int, fila: int):
    """Ver docstring de `procesar_ordenes_manuales`. Nunca vende más de
    lo que hay: si se pide más que la posición actual, se limita a esa
    cantidad y avisa la diferencia."""
    print(f"[ordenes_manuales] {ticker}: procesando VENTA de {cantidad_pedida} acciones (fila {fila})")
    niveles = leer_niveles_guardados(sheet)
    if ticker not in niveles:
        print(f"[ordenes_manuales] {ticker}: no hay posición abierta -- error")
        sheets.actualizar_orden_manual(sheet, fila, "error", f"no hay posición abierta en {ticker}")
        tg.notificar_error(f"venta manual {ticker}", "no hay posición abierta con ese ticker")
        return

    nivel = niveles[ticker]
    cantidad = min(cantidad_pedida, nivel["acciones"])
    nota_cantidad = "" if cantidad == cantidad_pedida else \
        f" (se pidieron {cantidad_pedida}, la posición solo tenía {nivel['acciones']})"

    resultado = _ejecutar_venta_manual(iol_client, sheet, ticker, nivel, cantidad,
                                        motivo="venta_manual", orden_manual_fila=fila)
    print(f"[ordenes_manuales] {ticker}: resultado venta -> {resultado['resultado']}")

    if resultado["resultado"] == "error":
        sheets.actualizar_orden_manual(sheet, fila, "error", resultado["error"])
        tg.notificar_error(f"venta manual {ticker}", resultado["error"])
        return

    if resultado["resultado"] == "pendiente":
        sheets.actualizar_orden_manual(
            sheet, fila, "en curso",
            f"orden de venta enviada ({cantidad} acciones), esperando confirmación{nota_cantidad}")
        return

    # confirmada al toque (cerrada o parcial)
    if resultado["resultado"] == "cerrada":
        del niveles[ticker]
    else:  # parcial
        niveles[ticker]["acciones"] -= cantidad
    _reescribir_operaciones_activas(iol_client, sheet, niveles)
    sheets.actualizar_orden_manual(
        sheet, fila, "ejecutada", f"vendidas {cantidad} a ${resultado['precio_salida']:,.2f}{nota_cantidad}")


def procesar_ordenes_manuales(iol_client, sheet):
    """
    *** ÓRDENES MANUALES (16/08/2026) *** -- ver pestaña "Órdenes
    Manuales" en Sheets. Permite forzar una compra o venta puntual por
    fuera de las reglas de la estrategia (una convicción propia, o
    corregir una posición cargada a mano). Se llama PRIMERO en las 3
    rutinas (apertura/refresh/cierre), antes que la cola de reintento y
    antes que las señales nuevas del día -- orden de prioridad pedido
    explícitamente: 1) Órdenes Manuales, 2) Señales Pendientes
    Ejecución, 3) señal nueva. Así una venta manual puede liberar
    capital para una compra (manual o de la cola) en la MISMA corrida,
    y una compra manual tiene prioridad de capital sobre todo lo demás.

    Cada fila se marca 'en curso' apenas se manda la orden (no cuando
    se confirma), para no volver a mandarla si la corrida siguiente
    encuentra la orden todavía sin confirmar -- eso lo termina de
    resolver `resolver_ordenes_pendientes()`, que deja la fila en
    'ejecutada' cuando confirma (ver datos_extra["orden_manual_fila"]).
    Si la fila queda en 'ejecutada' o 'error' y el usuario se olvida de
    borrarla, no se vuelve a procesar -- `leer_ordenes_manuales_pendientes`
    solo devuelve filas con Estado vacío o 'pendiente'.

    COMPRA: ignora TOPE_MAXIMO_POSICION (a pedido -- "sin límites, bajo
    mi responsabilidad") pero SIEMPRE respeta el efectivo real
    disponible: si se piden 5 y solo alcanza para 4, compra 4 y avisa
    la diferencia. Si no alcanza ni para 1, no compra nada (error). Si
    el ticker ya tiene posición abierta, se combina (ver
    `_registrar_compra_confirmada`): suma acciones y promedia el precio
    de entrada.

    VENTA: admite parcial. Si la cantidad pedida es menor a las
    acciones de la posición, vende esa porción y la fila de Operaciones
    Activas queda con el resto (mismos Fase/Stop). Si es igual o mayor,
    cierra la posición completa -- nunca vende de más de lo que hay. Si
    el ticker no tiene posición abierta, error (no inventa una venta de
    algo que no existe).

    *** LOGGING (19/08/2026) *** -- a diferencia de casi todo el resto
    del bot, esta función antes no imprimía NADA en el log cuando
    procesaba una orden con éxito (solo escribía en Sheets/Telegram) --
    eso hizo que un caso real de diagnóstico (TGNO4.BA cargada pero
    nunca ejecutada) fuera imposible de diagnosticar desde el log solo,
    porque "silencio" no distinguía entre "nunca vio la fila" y "la vio
    y algo raro pasó sin dejar rastro". Ahora cada paso imprime algo,
    éxito incluido.
    """
    ordenes = sheets.leer_ordenes_manuales_pendientes(sheet)
    if not ordenes:
        print("[ordenes_manuales] no hay órdenes pendientes en la pestaña 'Órdenes Manuales'")
        return
    print(f"[ordenes_manuales] {len(ordenes)} orden(es) pendiente(s) encontrada(s): "
          f"{[o['ticker'] for o in ordenes]}")

    for orden in ordenes:
        ticker = orden["ticker"]
        fila = orden["fila"]
        accion = orden["accion"]
        cantidad_pedida = orden["cantidad"]

        if accion not in ("comprar", "vender"):
            print(f"[ordenes_manuales] {ticker} (fila {fila}): acción inválida '{orden['accion']}'")
            sheets.actualizar_orden_manual(
                sheet, fila, "error", f"Acción inválida: '{orden['accion']}' (debe ser 'comprar' o 'vender')")
            continue
        if cantidad_pedida <= 0:
            print(f"[ordenes_manuales] {ticker} (fila {fila}): cantidad inválida ({cantidad_pedida})")
            sheets.actualizar_orden_manual(sheet, fila, "error", "Cantidad debe ser mayor a 0")
            continue

        if accion == "comprar":
            _procesar_compra_manual(iol_client, sheet, ticker, orden.get("tipo", ""), cantidad_pedida, fila)
        else:
            _procesar_venta_manual(iol_client, sheet, ticker, cantidad_pedida, fila)
...
def rutina_apertura(iol_client):
    ...
    procesar_ordenes_manuales(iol_client, sheet)  # ver docstring -- corre primero, siempre
    ...
    for orden in ordenes:
        ...
        if estado == "terminada" and cantidad and monto_operacion:
            ...
            if tipo_operacion == "venta":
                ...
                motivo_stop_default = f"stop_loss_{int(round(SL_INICIAL_PCT * 100))}pct"
                motivo = datos_extra.get("motivo_salida", motivo_stop_default)
                sheets.registrar_operacion_cerrada(
                    ...
                    orden["acciones"], datos_extra.get("sl_fase_a", 0), motivo,
                ...
                tg.notificar_cierre_posicion(ticker, hoy.strftime("%d/%m/%Y"), precio_real, motivo,
                                              desglose["pnl_pesos"], desglose["pnl_pct"])
                print(f"[refresh] {ticker}: venta pendiente CONFIRMADA a ${precio_real:,.2f}")

                # *** FIX 16/08/2026 -- esta rama NUNCA sacaba el ticker
                # de "Operaciones Activas" cuando la confirmación llegaba
                # recién en un refresh posterior (solo registraba el
                # cierre en el histórico). Quedaba mostrado como posición
                # abierta, con precio actualizado, hasta que algo más lo
                # corrigiera. Ahora se remueve (o se reduce si la venta
                # fue parcial) en el mismo paso. ***
                niveles_actuales = leer_niveles_guardados(sheet)
                if ticker in niveles_actuales:
                    if orden["acciones"] >= niveles_actuales[ticker]["acciones"]:
                        del niveles_actuales[ticker]
                    else:
                        niveles_actuales[ticker]["acciones"] -= orden["acciones"]
                    _reescribir_operaciones_activas(iol_client, sheet, niveles_actuales)

                orden_manual_fila = datos_extra.get("orden_manual_fila")
                if orden_manual_fila:
                    sheets.actualizar_orden_manual(
                        sheet, orden_manual_fila, "ejecutada",
                        f"vendidas {orden['acciones']} a ${precio_real:,.2f} (confirmada en refresh posterior)")
            else:  # compra
                niveles_actuales = _registrar_compra_confirmada(
                    sheet, ticker, orden.get("tipo", ""), precio_real, orden["acciones"], hoy)
                _reescribir_operaciones_activas(iol_client, sheet, niveles_actuales)
                tg.notificar_apertura_posicion(ticker, hoy.strftime("%d/%m/%Y"), precio_real,
                                                niveles_actuales[ticker]["sl_fase_a"], acciones=orden["acciones"])
                print(f"[refresh] {ticker}: compra pendiente CONFIRMADA a ${precio_real:,.2f}")

                orden_manual_fila = datos_extra.get("orden_manual_fila")
                if orden_manual_fila:
                    nota = datos_extra.get("orden_manual_nota", "")
                    sheets.actualizar_orden_manual(
                        sheet, orden_manual_fila, "ejecutada",
                        f"compradas {orden['acciones']} a ${precio_real:,.2f}{nota} (confirmada en refresh posterior)")
...
# ============================================================================
# RUTINA 2: MONITOREO FASE A (cada 10 min) -- solo SL 15% intradía
# ============================================================================
...
def rutina_monitoreo_fase_a(iol_client):
    ...
    for ticker in tickers_fase_a:
        ...
        motivo_stop = f"stop_loss_{int(round(SL_INICIAL_PCT * 100))}pct"
        if _cerrar_posicion(iol_client, sheet, ticker, nivel, motivo=motivo_stop,
...
# ============================================================================
@con_lock_rutina("cierre")
def rutina_cierre(iol_client):
    sheet = sheets.conectar_sheet()
    hoy = hoy_argentina()

    # Corre SIEMPRE, incluso si el resto de la rutina de cierre ya se
    # registró hoy -- una orden manual cargada después de las 16:45
    # tiene que poder ejecutarse en una corrida posterior del mismo día,
    # sin esperar al cierre de mañana. Prioridad pedida explícitamente:
    # 1) Órdenes Manuales, 2) Señales Pendientes Ejecución (más abajo,
    # bloque c1), 3) señal nueva (bloque c2).
    procesar_ordenes_manuales(iol_client, sheet)
    ...
    for ticker in list(niveles.keys()):
        ...
        ema10_hoy = df["EMA10"].iloc[-1] if "EMA10" in df.columns else None
        ...
        if nivel["fase"] == "A":
            if cierre_hoy > ema50_hoy:
                ...
                nivel["supero_umbral_lejos"] = False
                nivel["contador_ruptura_leve"] = 0
            ...
            # el motor RSI/BB -- SL fijo 10% hasta cruzar a Fase B; ver
            # diagnóstico de MFE en la bitácora del proyecto: no hay
            # señal temprana confiable que distinga ganadores de
            # perdedores, cualquier trailing probado empeoró el
            # resultado).
        else:
            # Fase B, y NO es el mismo día del cruce (si lo fuera, este
            # ticker habría entrado al `if` de arriba, no acá).
            #
            # *** SALIDA HÍBRIDA (06/08/2026) *** -- validada en backtest
            # sobre 102 tickers: 730.5% retorno (vs 701.5% del baseline
            # de 1 sola vela), max drawdown 4.6% (mejor que el baseline),
            # robusta ante concentración y en el período 2025-2026. Ver
            # bitácora del chat del proyecto para el detalle completo.
            #
            # 1) Si el cierre alguna vez estuvo >= UMBRAL_CORRIO_LEJOS_PCT
            #    por encima de la EMA50, la posición queda "marcada" para
            #    siempre (no se resetea si después baja) -- desde ahí,
            #    sale con el PRIMER cierre bajo EMA50 (sin esperar
            #    confirmación), igual que el comportamiento original.
            # 2) Si NUNCA corrió tan lejos: un cierre apenas por debajo
            #    (hasta UMBRAL_RUPTURA_FUERTE_PCT% bajo la EMA50) exige
            #    una segunda vela CONSECUTIVA para confirmar la salida
            #    (se resetea si en el medio cierra por encima). Una
            #    ruptura más fuerte que ese umbral sale de inmediato.
            if cierre_hoy >= ema50_hoy * (1 + UMBRAL_CORRIO_LEJOS_PCT / 100):
                nivel["supero_umbral_lejos"] = True

            # *** MODO EMA10 (10/08/2026) *** -- si el cierre alguna vez
            # estuvo >= UMBRAL_CORRIO_LEJOS_EMA10_PCT (60%) por encima de
            # la EMA50, la posición queda marcada para siempre -- desde
            # ahí, la ÚNICA regla de salida es EMA10, REEMPLAZANDO POR
            # COMPLETO la salida híbrida normal (ver bitácora del
            # proyecto, 10/08/2026, para la validación en backtest).
            if not nivel.get("modo_ema10") and cierre_hoy >= ema50_hoy * (1 + UMBRAL_CORRIO_LEJOS_EMA10_PCT / 100):
                nivel["modo_ema10"] = True

            debe_cerrar = False
            if nivel.get("modo_ema10"):
                if ema10_hoy and cierre_hoy <= ema10_hoy * (1 - DISTANCIA_SALIDA_EMA10_PCT / 100):
                    debe_cerrar = True
                motivo_cierre = "cierre_bajo_ema10"
            else:
                if cierre_hoy < ema50_hoy:
                    if nivel.get("supero_umbral_lejos"):
                        debe_cerrar = True
                    else:
                        distancia_bajo_pct = 100 * (ema50_hoy - cierre_hoy) / ema50_hoy
                        if distancia_bajo_pct > UMBRAL_RUPTURA_FUERTE_PCT:
                            debe_cerrar = True
                        else:
                            nivel["contador_ruptura_leve"] = nivel.get("contador_ruptura_leve", 0) + 1
                            if nivel["contador_ruptura_leve"] >= VELAS_CONFIRMACION_SALIDA:
                                debe_cerrar = True
                else:
                    nivel["contador_ruptura_leve"] = 0  # se resetea la racha de rupturas leves
                    nivel["stop_vigente"] = ema50_hoy  # el nivel de referencia sigue a la EMA50
                motivo_cierre = "cierre_bajo_ema50"

            if debe_cerrar:
                if _cerrar_posicion(iol_client, sheet, ticker, nivel, motivo=motivo_cierre,
    ...
    for item in cola_pendientes:
        ...
        if orden_compra.get("pendiente"):
            ...
            tg.notificar_orden_pendiente(ticker, "compra")
            # la señal SALE de la cola de reintento -- ya no es "sin capital",
            # está en curso en la cola de confirmación (resolver_ordenes_pendientes
            # se encarga de completarla o descartarla, no vuelve a esta cola).
            continue
    ...
    try:
        if not sheets.dashboard_de_hoy_ya_registrado(sheet):
            ...
            sheets.actualizar_dashboard_pnl(
                ...
                posiciones_activas=len(niveles),
            )
    except Exception as e:
        print(f"[cierre] no se pudo actualizar P&L Total: {e}")

    # *** RESUMEN DE CIERRE (19/08/2026, a pedido del usuario) *** --
    # mismo formato que el segundo mensaje de rutina_apertura, pero con
    # los números de DESPUÉS de procesar todo el cierre (si
    # indicadores_cierre quedó vacío por algún error de arriba, se
    # manda igual con los conteos en 0 -- mejor un resumen incompleto
    # que ninguno).
    tg.notificar_resumen_cierre(
        cantidad_tickers=len(indicadores_cierre),
        senales_pendientes=sum(1 for v in indicadores_cierre.values() if v["senal_pendiente"]),
        senales_confirmadas=sum(1 for v in indicadores_cierre.values() if v["senal_confirmada"]),
        en_cooldown=sum(1 for v in indicadores_cierre.values() if v["en_cooldown"]),
        posiciones_abiertas=len(niveles),
    )
...
def _reescribir_operaciones_activas(iol_client, sheet, niveles: dict):
    ...
    for ticker, nivel in niveles.items():
        ...
        filas[ticker] = {
            ...
            "supero_umbral_lejos": nivel.get("supero_umbral_lejos", False),
            "contador_ruptura_leve": nivel.get("contador_ruptura_leve", 0),
            "modo_ema10": nivel.get("modo_ema10", False),
        }
