...
class IOLClient:
    ...
    def _obtener_tick_size(self, ticker: str, mercado: str = "bCBA"):
        """
        Calcula el tick size REAL del instrumento a partir de la
        diferencia entre precios consecutivos de la caja de puntas
        (mismo endpoint de Cotizacion que ya usa obtener_precio, campo
        "puntas" -- lista de {precioCompra, cantidadCompra, precioVenta,
        cantidadVenta}). Evita el método de prueba-y-error de
        _REDONDEOS_PRECIO cuando el dato está disponible: en vez de
        adivinar, se usa el múltiplo real observado en el mercado.

        *** SIN CONFIRMAR AL 100% *** -- el nombre exacto del campo
        "puntas" (y sus subcampos) se infiere de la estructura típica
        de la API de IOL, pero no se pudo verificar en un entorno con
        acceso real a la API al escribir esto. Si el campo no existe o
        tiene otro nombre, esta función devuelve None sin romper nada
        -- el llamador cae al método de redondeo por prueba-y-error de
        siempre (_REDONDEOS_PRECIO) como respaldo, así que no hay
        riesgo de que esto rompa una compra si la detección falla.

        Devuelve el tick size (float) o None si no se pudo determinar.
        """
        try:
            simbolo = ticker.replace(".BA", "")
            cotizacion = self._get(f"/api/v2/{mercado}/Titulos/{simbolo}/Cotizacion")
            puntas = cotizacion.get("puntas") or cotizacion.get("Puntas") or []
            if len(puntas) < 2:
                return None

            precios_compra = sorted(
                {p["precioCompra"] for p in puntas if p.get("precioCompra")}, reverse=True
            )
            precios_venta = sorted(
                {p["precioVenta"] for p in puntas if p.get("precioVenta")}
            )

            diferencias = []
            for lista in (precios_compra, precios_venta):
                for i in range(len(lista) - 1):
                    diferencias.append(round(abs(lista[i] - lista[i + 1]), 6))

            diferencias_positivas = [d for d in diferencias if d > 0]
            if not diferencias_positivas:
                return None
            return min(diferencias_positivas)
        except Exception as e:
            print(f"[iol_client] no se pudo determinar tick size real para {ticker}: {e}")
            return None
    ...
    def comprar_mercado(self, ticker: str, cantidad: int, mercado: str = "bCBA",
        ...
        *** AJUSTADO TRAS TEST REAL #2 ***
        ...
        tick_real = self._obtener_tick_size(ticker, mercado=mercado)
        redondeos_a_probar = list(self._REDONDEOS_PRECIO)
        if tick_real:
            redondeos_a_probar = [lambda p, t=tick_real: round(p / t) * t] + redondeos_a_probar

        for redondeo in redondeos_a_probar:
    ...
    _REDONDEOS_PRECIO = [
        ...
        lambda p: round(p / 10) * 10,
        lambda p: round(p / 20) * 20,   # confirmado con la caja de puntas real de TSLA.BA (28/07/2026)
        lambda p: round(p / 25) * 25,
        lambda p: round(p / 50) * 50,
        lambda p: round(p / 100) * 100,
    ]
    ...
    def _interpretar_respuesta_orden(self, respuesta: dict, intentos: int = 5,
        ...
        if precio_ejecutado is None:
            ...
            # No se pudo confirmar la ejecución tras varios intentos --
            # la orden puede seguir PENDIENTE en el libro (no rechazada,
            # pero tampoco confirmada como ejecutada). Antes esto se
            # marcaba como "exito": True con un precio estimado -- el
            # bot avisó "cerrado" y lo sacó de seguimiento, pero la
            # orden real seguía pendiente en IOL y recién se ejecutó
            # más tarde, sin que el bot la monitoreara mientras tanto
            # (mensaje de Telegram con un PNL calculado sobre un precio
            # que en realidad todavía no era el ejecutado).
            #
            # Ahora se devuelve `exito=False, pendiente=True` -- el
            # llamador NO debe dar la posición por cerrada/abierta con
            # esto; debe guardar `numero_operacion` y resolverlo más
            # tarde (ver rutina_refresh() -> resolver_ordenes_pendientes
            # en bot_bb_touch_diario.py, que reintenta la confirmación
            # en cada refresh de las 13hs/16hs sin volver a mandar una
            # orden nueva).
            return {
                "exito": False,
                "pendiente": True,
                "numero_operacion": numero_operacion,
                "precio_estimado": respuesta.get("precio", 0),
                "error": "Orden enviada, todavía no se pudo confirmar si se ejecutó -- reintentar más tarde",
            }
