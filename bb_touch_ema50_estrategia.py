...
def generar_senales_bb_touch_bbw(df: pd.DataFrame, bbw_umbral: float = BBW_UMBRAL) -> pd.DataFrame:
    ...
    cada toque nuevo, igual que el resto de la lógica de la señal) --
    permite mostrar en el dashboard hace cuánto viene esperando una
    señal, y de dónde salió el precio de referencia para la cola de
    reintento de ejecución (ver `bot_bb_touch_diario.py`).
    """
    df = df.copy()
