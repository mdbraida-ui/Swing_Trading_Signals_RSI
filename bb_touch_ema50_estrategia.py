...
def generar_senales_bb_touch_bbw(df: pd.DataFrame, bbw_umbral: float = BBW_UMBRAL,
    ...
    *** FILTRO DE MOVIMIENTO MÁXIMO (llevado a producción tras validar
    ...
    Sobre el universo real de 102 tickers (2023-2026), este filtro
    subió el win rate de 46.6%->49.9% y bajó el max drawdown de
    8.1%->6.6%, a cambio de un retorno bruto algo menor (605%->529%) --
    ver bitácora de decisión en el chat del proyecto.

    Agrega `fecha_toque_vigente`: la fecha del toque de banda MÁS
    RECIENTE que originó el tramo "pendiente" actual (se reinicia con
    cada toque nuevo) -- permite mostrar en el dashboard hace cuánto
    viene esperando una señal, y de dónde salió el precio de
    referencia para la cola de reintento de ejecución (ver
    `bot_bb_touch_diario.py`).

    Agrega `senal_invalidada_por_movimiento`: True en el día puntual
    en que una señal pendiente se descartó por este filtro (informativo
    / debug, no se usa para decidir nada más adelante en el pipeline).
    """
    df = df.copy()
...
def forzar_cierre_de_hoy(df: pd.DataFrame, precio_en_vivo: float, fecha_hoy) -> pd.DataFrame:
    ...
    *** AJUSTADO TRAS DATO SUCIO REAL (AAP.BA, 27/07/2026) ***
    ...
    Como de todos modos solo tenemos UN dato en vivo por corrida (no un
    historial intradiario real de IOL), no tiene sentido combinarlo con
    el Low/High de Yahoo -- directamente se REEMPLAZA todo el OHLC de
    hoy por el precio en vivo. Es más honesto con lo que sabemos de
    verdad ("a esta hora, el precio es X"), sin heredar un dato
    intradiario de Yahoo que no podemos verificar.
    """
    df = df.copy()
    if len(df) and df.index[-1].date() == fecha_hoy:
        df.loc[df.index[-1], ["Open", "High", "Low", "Close"]] = precio_en_vivo
    else:
