...
Setup necesario (una sola vez):
  ...
  "Operaciones Activas":
      ...
      P&L % | Días en Posición | Superó 15% | Contador Ruptura Leve |
      Modo EMA10
...
def actualizar_operaciones_activas(sheet, posiciones: dict):
    ...
    `posiciones`: dict ticker -> {
        ...
        "supero_umbral_lejos": bool, "contador_ruptura_leve": int,
        "modo_ema10": bool,
    }
    ...
    "Superó 15%" / "Contador Ruptura Leve": estado de la salida HÍBRIDA
    de Fase B (agregada 06/08/2026) -- se persiste acá porque el bot no
    guarda nada en memoria entre corridas. Ver rutina_cierre() en
    bot_bb_touch_diario.py para la lógica completa.
    """
    if sheet is None:
        return
    try:
        ...
        filas = [["Ticker", "Tipo", "Fecha Entrada", "Precio Entrada", "Acciones",
                   ...
                   "P&L $", "P&L %", "Días en Posición", "Superó 15%", "Contador Ruptura Leve",
                   "Modo EMA10"]]
        for ticker, p in sorted(posiciones.items()):
            ...
            filas.append([
                ...
                "SI" if p.get("supero_umbral_lejos") else "no",
                p.get("contador_ruptura_leve", 0),
                "SI" if p.get("modo_ema10") else "no",
            ])
...
# ============================================================================
# 10) ÓRDENES MANUALES -- compra/venta forzada por el usuario, por fuera
#     de las reglas de la estrategia (agregado 16/08/2026)
# ============================================================================
def leer_ordenes_manuales_pendientes(sheet) -> list:
    """Lee la pestaña 'Órdenes Manuales' y devuelve solo las filas con
    Estado en blanco o 'pendiente' (no distingue mayúsculas) -- las que
    ya se procesaron ('ejecutada', 'en curso', 'error') se ignoran, así
    que dejar la fila sin borrar después de ejecutada NO la vuelve a
    disparar. Cada elemento incluye `fila` (número de fila real en la
    hoja, 1-indexado, para poder actualizarla puntualmente después sin
    reescribir toda la pestaña)."""
    if sheet is None:
        return []
    try:
        ws = sheet.worksheet("Órdenes Manuales")
        registros = ws.get_all_records(value_render_option='UNFORMATTED_VALUE')
    except Exception as e:
        print(f"[sheets] no se pudo leer Órdenes Manuales: {e}")
        return []
    ordenes = []
    for i, r in enumerate(registros):
        estado = str(r.get("Estado", "")).strip().lower()
        if estado not in ("", "pendiente"):
            continue  # ya procesada -- se ignora
        try:
            ticker = str(r["Ticker"]).strip()
            cantidad = int(r["Cantidad"])
        except (KeyError, ValueError) as e:
            print(f"[sheets] fila inválida en Órdenes Manuales (fila {i + 2}), se ignora: {e}")
            continue
        if not ticker:
            continue
        ordenes.append({
            "fila": i + 2,  # +2: fila 1 es encabezado, get_all_records es 0-indexado
            "ticker": ticker,
            "tipo": str(r.get("Tipo", "")).strip(),
            "accion": str(r.get("Acción", "")).strip().lower(),
            "cantidad": cantidad,
        })
    return ordenes


def actualizar_orden_manual(sheet, fila: int, estado: str, notas: str = ""):
    """Actualiza puntualmente Estado (columna E) y Notas (columna G) de
    una fila de 'Órdenes Manuales', y completa Fecha Solicitud (columna
    F) con la fecha/hora actual si todavía estaba vacía -- así queda
    registro de cuándo se procesó, sin que el usuario tenga que
    cargarla a mano. Actualiza celda por celda (no toda la fila) para
    no pisar Ticker/Tipo/Acción/Cantidad que cargó el usuario."""
    if sheet is None:
        return
    try:
        ws = sheet.worksheet("Órdenes Manuales")
        fecha_actual = ws.cell(fila, 6).value  # columna F = Fecha Solicitud
        if not fecha_actual:
            ws.update_cell(fila, 6, _ahora_str())
        ws.update_cell(fila, 5, estado)  # columna E = Estado
        ws.update_cell(fila, 7, notas)   # columna G = Notas
    except Exception as e:
        print(f"[sheets] error al actualizar Órdenes Manuales (fila {fila}): {e}")
...
# ============================================================================
# 11) LOCK CONTRA CORRIDAS SUPERPUESTAS (19/08/2026)
# ============================================================================
NOMBRE_HOJA_LOCK = "Lock"
TIMEOUT_LOCK_MINUTOS = 15  # de sobra -- la corrida más larga observada anduvo por los 5 min


def intentar_tomar_lock(sheet, nombre_rutina: str) -> bool:
    """
    *** LOCK CONTRA CORRIDAS SUPERPUESTAS (19/08/2026) -- bug real en
    producción ***
    El 19/08/2026 `rutina_cierre` corrió dos veces casi al mismo
    tiempo: el respaldo nativo de GitHub (`schedule:`, corre cada 5 min
    "por si cron-job.org falla alguna vez") coincidió con el disparo
    real de cron-job.org dentro de la misma ventana de cierre. Las dos
    pasaron el chequeo de "¿ya se registró hoy?" porque ninguna había
    terminado de escribir todavía -- no es un lock real, es solo un
    chequeo de si YA se escribió, no de si alguien más lo está
    escribiendo AHORA. La que terminó después pisó lo que había
    escrito la primera: una posición real comprada (LOMA.BA) desapareció
    de "Operaciones Activas" aunque en IOL sí estaba.

    Esta función usa una pestaña "Lock" de una sola fila de datos (A2:
    nombre de la rutina que lo tiene tomado, B2: desde cuándo) como
    lock real. Se llama ANTES de hacer cualquier trabajo, en las 4
    rutinas. No es 100% atómico (Sheets no tiene compare-and-swap), pero
    reduce la ventana de carrera de "toda la duración de la corrida"
    (~3-5 min, la causa del bug real) a "una sola lectura+escritura"
    (~1 segundo) -- suficiente en la práctica para el volumen de
    corridas de este bot.

    Si el lock lleva tomado más de TIMEOUT_LOCK_MINUTOS, se considera
    abandonado (la corrida anterior se colgó o crasheó sin liberarlo) y
    se toma igual -- para no dejar el bot bloqueado para siempre por un
    cuelgue puntual.

    Devuelve True si se tomó el lock (hay que liberarlo con
    `liberar_lock` al terminar, en un finally). Devuelve False si ya
    estaba tomado por otra corrida en curso -- en ese caso NO hay que
    liberar nada (no se tomó).

    Si la pestaña "Lock" no existe o hay un error de Sheets, se falla
    "abierto" (devuelve True, deja seguir sin lock) -- un problema en
    esta pestaña puntual no debería frenar al bot entero de operar.
    """
    try:
        ws = sheet.worksheet(NOMBRE_HOJA_LOCK)
    except Exception as e:
        print(f"[lock] no se pudo acceder a la pestaña Lock, se sigue SIN lock: {e}")
        return True
    ...
    try:
        ...
        if len(valores) >= 2 and len(valores[1]) >= 2 and valores[1][0]:
            ...
            try:
                ...
                minutos_transcurridos = (datetime.now(TZ_ARGENTINA).replace(tzinfo=None) - timestamp).total_seconds() / 60
            except ValueError:
                minutos_transcurridos = TIMEOUT_LOCK_MINUTOS + 1  # timestamp corrupto -- se trata como abandonado
            ...
            if minutos_transcurridos < TIMEOUT_LOCK_MINUTOS:
                ...
                return False
            print(f"[lock] lock de '{rutina_actual}' abandonado hace {minutos_transcurridos:.1f} min "
                  f"-- se toma igual")

        ws.update(range_name="A1:B2",
                  values=[["Rutina", "Desde"], [nombre_rutina, datetime.now(TZ_ARGENTINA).strftime("%Y-%m-%d %H:%M:%S")]])
        return True
    except Exception as e:
...
def liberar_lock(sheet):
    """Libera el lock -- SIEMPRE se llama en un finally, así se libera
    incluso si la rutina explota a mitad de camino."""
    try:
        ws = sheet.worksheet(NOMBRE_HOJA_LOCK)
