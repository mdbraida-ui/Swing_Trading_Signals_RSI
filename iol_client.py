...
class IOLClient:
    ...
    # ------------------------------------------------------------------
    def comprar_mercado(self, ticker: str, cantidad: int, mercado: str = "bCBA") -> dict:
        """
        Envía una orden de compra a PRECIO DE MERCADO (Contado Inmediato).
        Devuelve {"exito": bool, "precio_ejecutado": float, "numero_operacion": ...}

        *** CAMPOS AJUSTADOS TRAS PRIMER TEST REAL ***
        El primer intento con "validez": None y "tipoOrden": "mercado"
        devolvió un error genérico y mal formado ('{"message": " - "}'),
        consistente con el backend rompiéndose al intentar interpolar un
        campo null en el mensaje de error. Se saca "tipoOrden" (campo no
        confirmado, podría ni existir) y se manda una fecha real en
        "validez" en vez de null.
        """
        simbolo = ticker.replace(".BA", "")
        ...
        body = {
            ...
            "simbolo": simbolo,
            "cantidad": cantidad,
            "precio": 0,          # 0 = a precio de mercado
    ...
    def vender_mercado(self, ticker: str, cantidad: int, mercado: str = "bCBA") -> dict:
        simbolo = ticker.replace(".BA", "")
        from datetime import date
        body = {
            "mercado": mercado,
