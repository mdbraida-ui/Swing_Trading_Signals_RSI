...
RUTA_CONFIG = "config_bot_opciones.csv"
EMA_REFERENCIA = 18
...
ENCABEZADOS_HISTORICO = ["fecha_entrada", "fecha_salida", "ticker", "tipo", "simbolo_opcion",
                          ...
                          "motivo_entrada", "motivo_salida"]
ENCABEZADOS_PENDIENTES = ["fecha", "ticker", "tipo_orden", "simbolo_opcion", "numero_operacion",
                           "cantidad_contratos", "detalle", "motivo"]
# motivo (Posicion/Historico/Pendientes), valores usados en este bot:
#   "senal_automatica"  -- entrada por cruce EMA4/18 (PASO 2)
#   "cruce_ema4_18"     -- salida por cruce EMA4/18 (PASO 1)
#   "roleo"             -- compra/venta generada por el mecanismo de roleo
#   "manual"            -- cargada a mano en "Ordenes Manuales Opciones"
#   "pendiente_confirmada_tarde" -- se agrega en resolver_ordenes_pendientes
#   cuando una orden que había quedado sin confirmar termina resolviéndose
...
def leer_ordenes_pendientes(pestana) -> list:
    """Lee todas las filas de 'Ordenes Pendientes Opciones' -- a
    diferencia de las manuales, acá NO hay columna de estado: cada fila
    que existe está, por definición, sin resolver todavía.

    *** BUG REAL QUE ESTO CORRIGE (09/09/2026) *** -- antes exigía
    len(fila) == len(ENCABEZADOS_PENDIENTES) exacto. Cuando se agregó
    la columna 'motivo' (pasando de 7 a 8 encabezados), las filas VIEJAS
    (con 7 columnas, escritas por una versión anterior del bot) dejaron
    de cumplir esa condición y quedaron invisibles para el resolver --
    una orden de venta real, ya confirmada en IOL, se quedó "pendiente"
    en Sheets para siempre porque esta función ni siquiera la leía."""
    valores = pestana.get_all_values()
    pendientes = []
    for fila in valores[1:]:
        if len(fila) >= 1 and fila[0]:
            if len(fila) < len(ENCABEZADOS_PENDIENTES):
                fila = fila + [""] * (len(ENCABEZADOS_PENDIENTES) - len(fila))
            pendientes.append(dict(zip(ENCABEZADOS_PENDIENTES, fila)))
    return pendientes


def reescribir_ordenes_pendientes(pestana, ordenes_que_siguen: list):
    """Reemplaza toda la pestaña con SOLO las órdenes que siguen sin
    resolver -- las que se resolvieron (bien o mal) desaparecen."""
    pestana.clear()
    pestana.append_row(ENCABEZADOS_PENDIENTES)
    for orden in ordenes_que_siguen:
        pestana.append_row([orden.get(c, "") for c in ENCABEZADOS_PENDIENTES])


def resolver_ordenes_pendientes(cliente_iol: IOLClient, pestana_pendientes, pestana_posicion,
                                 pestana_historico, posiciones_activas: dict):
    """
    *** RESUELVE EL AGUJERO REAL DEL 09/09/2026 *** -- calcado de
    `resolver_ordenes_pendientes()` en bot_bb_touch_diario.py (el bot
    de acciones), que ya resolvió este mismo problema ahí. Antes de
    esto, una orden que quedaba "pendiente" (enviada pero sin
    confirmar en los 5 reintentos inmediatos) se escribía en esta
    pestaña y NUNCA se volvía a revisar -- lo cual generó la confusión
    real del 09/09 (venta de roleo pendiente + compra manual encima del
    mismo símbolo, sin saber si la venta original había terminado
    ejecutándose).

    Se llama al PRINCIPIO de cada corrida, antes de evaluar cualquier
    señal nueva -- así una orden que tardó en confirmarse ya tiene
    chance de haberse resuelto en el mercado real para cuando este bot
    vuelve a mirarla.
    """
    ordenes = leer_ordenes_pendientes(pestana_pendientes)
    if not ordenes:
        return
    ...
    for orden in ordenes:
        ticker = orden["ticker"]
        tipo_orden = orden["tipo_orden"]  # "venta", "compra", "venta (roleo)", "compra (roleo)"
        ...
        try:
            detalle = cliente_iol.consultar_detalle_operacion(numero_operacion)
        except Exception as e:
            print(f"[pendientes] {ticker}: error al consultar {numero_operacion} ({e})")
        ...
        if estado == "terminada" and cantidad and monto_operacion:
            ...
            print(f"[pendientes] {ticker}: {tipo_orden} de {simbolo_opcion} CONFIRMADA a ${precio_real:,.2f}")

            if "venta" in tipo_orden:
                posicion_actual = posiciones_activas.get(ticker)
                if posicion_actual and posicion_actual.get("simbolo_opcion") == simbolo_opcion:
                    precio_entrada = float(posicion_actual.get("precio_entrada", precio_real))
                    monto_entrada = precio_entrada * cantidad
                    monto_salida = precio_real * cantidad
                    ...
                    registrar_historico(pestana_historico, {
                        "fecha_entrada": posicion_actual.get("fecha_entrada", ""), "fecha_salida": hoy_str,
                        "ticker": ticker, "tipo": posicion_actual.get("tipo", ""), "simbolo_opcion": simbolo_opcion,
                        "strike": posicion_actual.get("strike", ""), "precio_entrada": precio_entrada,
                        "precio_salida": precio_real, "cantidad_contratos": cantidad_contratos,
                        "pnl_pesos": pnl_pesos, "pnl_pct": pnl_pct,
                        "numero_operacion_compra": posicion_actual.get("numero_operacion_compra", ""),
                        "numero_operacion_venta": numero_operacion,
                        "motivo_entrada": posicion_actual.get("motivo_entrada", ""),
                        "motivo_salida": "pendiente_confirmada_tarde",
                    })
                    quitar_posicion(pestana_posicion, ticker)
                    posiciones_activas.pop(ticker, None)
                    tg.notificar_cierre_posicion(simbolo_opcion, hoy_str, precio_real,
                                                  f"{tipo_orden} (confirmada tarde)", pnl_pesos, pnl_pct)
                else:
                    tg._enviar(
                        f"⚠️ Venta de {simbolo_opcion} ({ticker}) confirmada tarde a ${precio_real:,.2f}, "
                        f"pero ya no hay una posición activa registrada con ese símbolo -- revisar a mano.",
                        usar_markdown=False
                    )
            else:  # compra (roleo, señal automática, o manual que quedó pendiente)
                strike_derivado = extraer_strike_de_simbolo(simbolo_opcion)
                tipo_derivado = derivar_tipo_de_simbolo(simbolo_opcion)
                fila_posicion = {
                    "ticker": ticker, "tipo": tipo_derivado or "", "simbolo_opcion": simbolo_opcion,
                    "strike": strike_derivado, "fecha_entrada": hoy_str,
                    "precio_entrada": precio_real, "cantidad_contratos": cantidad_contratos,
                    "numero_operacion_compra": numero_operacion,
                    "motivo_entrada": orden.get("motivo", "pendiente_confirmada_tarde"),
                }
                quitar_posicion(pestana_posicion, ticker)  # por si quedó algo viejo del mismo ticker
                agregar_posicion(pestana_posicion, fila_posicion)
                posiciones_activas[ticker] = fila_posicion
                tg._enviar(
                    f"✅ Compra de {simbolo_opcion} ({ticker}) confirmada tarde a ${precio_real:,.2f} "
                    f"-- posición registrada en Sheets automáticamente.",
                    usar_markdown=False
                )
            continue  # se resolvió -- no vuelve a la cola

        if "rechaz" in estado or "cancel" in estado:
            tg.notificar_error(f"Orden pendiente rechazada/cancelada -- {ticker}",
                                f"{tipo_orden} de {simbolo_opcion}, estado final: {estado}")
            print(f"[pendientes] {ticker}: orden pendiente terminó {estado} -- se descarta")
            continue  # se resolvió (mal) -- tampoco vuelve a la cola

        # sigue sin resolverse
        print(f"[pendientes] {ticker}: {tipo_orden} de {simbolo_opcion} sigue pendiente")
        ordenes_que_siguen_pendientes.append(orden)

    reescribir_ordenes_pendientes(pestana_pendientes, ordenes_que_siguen_pendientes)


def extraer_strike_de_simbolo(simbolo: str):
    """Deriva el strike numérico de un símbolo de opción (ej.
    'GFGV6800OC' -> 6800.0, 'COMC43.0OC' -> 43.0) -- usado para las
    órdenes MANUALES, cuya pestaña no tiene columna de strike propia.
    Devuelve None si no encuentra ningún número en el símbolo."""
    match = re.search(r'[\d.]+', simbolo)
    return float(match.group()) if match else None


def derivar_tipo_de_simbolo(simbolo: str):
    """
    Confirmado empíricamente en los 4 tickers de este proyecto: la 4ta
    letra del símbolo (índice 3) es 'C' para CALL o 'V' para PUT --
    GFGC/GFGV, PAMC/PAMV, YPFC/YPFV, COMC/COMV. Devuelve 'call', 'put'
    o None si no matchea el patrón esperado.

    *** BUG REAL QUE ESTO PREVIENE (09/09/2026) *** -- una orden manual
    se cargó con tipo='call' para el símbolo GFGV6600OC, que en
    realidad es un PUT (la 'V'). Sin esta validación, la posición
    quedó mal etiquetada -- la próxima evaluación de salida hubiera
    chequeado el cruce de EMA equivocado para esa posición.
    """
    if len(simbolo) < 4:
        return None
    letra = simbolo[3].upper()
    if letra == "C":
        return "call"
    if letra == "V":
        return "put"
...
NOMBRE_PESTANA_RESERVORIO = "Reservorio Precios IOL"
ENCABEZADOS_RESERVORIO = ["ticker", "fecha_hora", "precio"]
MAX_LECTURAS_POR_TICKER = 250  # ventana móvil -- ~36 días hábiles con
    # 7 lecturas/día (11:25 a 16:55). Corre EN PARALELO con Yahoo -- no
    # reemplaza la fuente de datos de la señal todavía, solo acumula
    # historial propio para el día que se decida migrar.  # mismo valor que el bot de acciones -- de sobra
...
def registrar_lectura_precio(pestana, ticker: str, precio: float):
    """
    Guarda una lectura de precio nueva para `ticker` (solo el número,
    no OHLC completo -- la señal en producción solo usa Close, así que
    no hace falta reconstruir una vela horaria). Si ese ticker ya tiene
    más de MAX_LECTURAS_POR_TICKER lecturas guardadas, poda las más
    viejas -- ventana móvil, no crece para siempre.

    *** BUG REAL QUE ESTO CORRIGE (09/09/2026) *** -- usaba
    datetime.datetime.now() sin zona horaria. GitHub Actions corre en
    UTC, no en hora Argentina -- las lecturas quedaban guardadas 3
    horas adelantadas (ej. "14:25" en vez de "11:25").
    """
    from zoneinfo import ZoneInfo
    ahora_ar = datetime.datetime.now(ZoneInfo("America/Argentina/Buenos_Aires"))
    pestana.append_row([ticker, ahora_ar.strftime("%Y-%m-%d %H:%M:%S"), precio])

    valores = pestana.get_all_values()
    filas_de_este_ticker = [i for i, fila in enumerate(valores[1:], start=2)
                              if len(fila) >= 1 and fila[0] == ticker]

    if len(filas_de_este_ticker) > MAX_LECTURAS_POR_TICKER:
        exceso = len(filas_de_este_ticker) - MAX_LECTURAS_POR_TICKER
        indices_a_borrar = set(filas_de_este_ticker[:exceso])  # las más viejas de ESTE ticker

        nuevas_filas = [valores[0]]  # encabezado
        ...
        print(f"[reservorio] {ticker}: podadas {exceso} lecturas viejas "
              f"(quedan {MAX_LECTURAS_POR_TICKER})")


def leer_reservorio(pestana, ticker: str) -> list:
    """Devuelve [(fecha_hora_str, precio), ...] para un ticker, en
    orden cronológico (mismo orden en que se insertaron). Todavía no
    se usa para calcular la señal -- queda lista para el día que se
    decida migrar de Yahoo a esta fuente."""
    valores = pestana.get_all_values()
    return [(fila[1], float(fila[2])) for fila in valores[1:]
            if len(fila) >= 3 and fila[0] == ticker]
...
def determinar_modo_rutina() -> str:
    ...
    'apertura', 'control' o 'cierre'. Prioridad:
      ...
      2) Si no está seteada, se infiere por la hora actual en
         Argentina -- apertura=10:40, cierre=16:55 (última corrida del
         día, opera igual que un control y además notifica
         desconexión), cualquier otra hora=control.
    """
    ...
    if abs(hora_min - (10 * 60 + 40)) <= 4:  # margen de 4 min por si el cron se atrasa
        return "apertura"
    if abs(hora_min - (16 * 60 + 55)) <= 4:
        return "cierre"
    return "control"


PUNTOS_DE_LECTURA_RESERVORIO = [
    (11, 25), (12, 25), (13, 25), (14, 25), (15, 25), (16, 25), (16, 55),
]


def corresponde_leer_reservorio_ahora() -> bool:
    """
    *** BUG REAL QUE ESTO CORRIGE (09/09/2026) *** -- antes se
    registraba una lectura en CUALQUIER corrida clasificada como
    'control', sin chequear la hora real. El cron interno de GitHub
    (respaldo redundante de cron-job.org) puede disparar tarde y en
    cualquier horario -- eso generó lecturas espurias a las 17:15 y
    17:53, que no coinciden con ninguno de los 7 puntos reales de
    control (11:25 a 16:55). Esta función exige coincidir con alguno
    de esos puntos (±4 min de margen), sin importar que el modo general
    de la corrida sea 'control'."""
    from zoneinfo import ZoneInfo
    ahora = datetime.datetime.now(ZoneInfo("America/Argentina/Buenos_Aires"))
    hora_min = ahora.hour * 60 + ahora.minute
    for h, m in PUNTOS_DE_LECTURA_RESERVORIO:
        if abs(hora_min - (h * 60 + m)) <= 4:
            return True
    return False
...
def _ejecutar_bot(planilla):
    ...
    pestana_reservorio = obtener_o_crear_pestana(planilla, NOMBRE_PESTANA_RESERVORIO, ENCABEZADOS_RESERVORIO)
    ...
    # ------------------------------------------------------------------
    # PASO -1: resolver órdenes que quedaron PENDIENTES en corridas
    # anteriores -- ANTES de mirar nada más, para no repetir la
    # confusión del 09/09/2026 (venta de roleo pendiente sin resolver +
    # intervención manual encima, sin saber el estado real).
    # ------------------------------------------------------------------
    resolver_ordenes_pendientes(cliente_iol, pestana_pendientes, pestana_posicion,
                                 pestana_historico, posiciones_activas)

    # ------------------------------------------------------------------
    # RESERVORIO DE PRECIOS -- registra el precio en vivo de IOL de
    # cada ticker activo, en cada control/cierre (no en apertura, que
    # no coincide con un cierre de vela de 1h). Corre EN PARALELO con
    # Yahoo -- no cambia la fuente de datos de la señal todavía, solo
    # va acumulando historial propio para una futura migración.
    # ------------------------------------------------------------------
    if modo in ("control", "cierre") and corresponde_leer_reservorio_ahora():
        for fila_config in config:
            ticker = fila_config["ticker_yahoo"]
            try:
                precio_actual = cliente_iol.obtener_precio(ticker, mercado="bCBA")
                registrar_lectura_precio(pestana_reservorio, ticker, precio_actual)
            except Exception as e:
                print(f"[reservorio] {ticker}: error al registrar lectura ({e})")
    elif modo in ("control", "cierre"):
        print("[reservorio] corrida fuera de los 7 puntos horarios esperados -- no se registra lectura "
              "(probablemente el cron interno de GitHub disparando tarde como respaldo)")
    ...
    for orden in leer_ordenes_manuales_pendientes(pestana_manuales):
        ...
        simbolo_opcion = orden["simbolo_opcion"]
        tipo_derivado = derivar_tipo_de_simbolo(simbolo_opcion)
        if tipo_derivado and tipo_derivado != tipo.strip().lower():
            print(f"[MANUAL] AVISO: tipo ingresado '{tipo}' no coincide con el símbolo "
                  f"'{simbolo_opcion}' (parece ser '{tipo_derivado}') -- se usa el derivado del símbolo.")
            tg.notificar_error(
                f"Orden manual con tipo incorrecto -- {ticker}",
                f"Cargaste tipo='{tipo}' para el símbolo {simbolo_opcion}, pero ese símbolo "
                f"corresponde a '{tipo_derivado}'. Se usó '{tipo_derivado}' para no guardar la "
                f"posición mal etiquetada -- revisá la fila en Ordenes Manuales."
            )
            tipo = tipo_derivado
        try:
            cantidad_contratos = int(orden["cantidad_contratos"])
        ...
        if resultado.get("pendiente"):
            marcar_orden_manual(pestana_manuales, orden["_fila_numero"], "pendiente_confirmacion",
                                 resultado.get("error", ""))
            tipo_orden_normalizado = "compra" if accion == "comprar" else "venta"
            registrar_pendiente(pestana_pendientes, {
                "fecha": hoy_str, "ticker": ticker, "tipo_orden": tipo_orden_normalizado,
                "simbolo_opcion": simbolo_opcion, "numero_operacion": resultado["numero_operacion"],
                "cantidad_contratos": cantidad_contratos, "detalle": resultado.get("error", ""),
                "motivo": "manual",
            })
        ...
        if accion == "comprar":
            quitar_posicion(pestana_posicion, ticker)  # evita filas duplicadas del mismo ticker
            fila_posicion = {
                "ticker": ticker, "tipo": tipo, "simbolo_opcion": simbolo_opcion,
                "strike": extraer_strike_de_simbolo(simbolo_opcion), "fecha_entrada": hoy_str,
                "precio_entrada": precio_ejecutado, "cantidad_contratos": cantidad_contratos,
                "numero_operacion_compra": resultado["numero_operacion"],
                "motivo_entrada": "manual",
            }
            agregar_posicion(pestana_posicion, fila_posicion)
            posiciones_activas[ticker] = fila_posicion  # dict COMPLETO -- no solo {"tipo": tipo}
        else:  # vender manual -- CIERRA la posición conocida, registrando
               # histórico con PnL real (antes esta rama no registraba
               # nada en el histórico -- se perdía el resultado de toda
               # venta manual)
            posicion_existente = posiciones_activas.get(ticker)
            if posicion_existente:
                precio_entrada_previo = float(posicion_existente.get("precio_entrada", precio_ejecutado))
                monto_entrada = precio_entrada_previo * cantidad_api
                monto_salida = precio_ejecutado * cantidad_api
                comision_pct = 0.005 * 1.21
                costos = monto_entrada * comision_pct + monto_salida * comision_pct
                pnl_pesos = (monto_salida - monto_entrada) - costos
                pnl_pct = 100 * pnl_pesos / monto_entrada if monto_entrada else 0
                registrar_historico(pestana_historico, {
                    "fecha_entrada": posicion_existente.get("fecha_entrada", ""), "fecha_salida": hoy_str,
                    "ticker": ticker, "tipo": posicion_existente.get("tipo", tipo),
                    "simbolo_opcion": simbolo_opcion, "strike": posicion_existente.get("strike", ""),
                    "precio_entrada": precio_entrada_previo, "precio_salida": precio_ejecutado,
                    "cantidad_contratos": cantidad_contratos, "pnl_pesos": pnl_pesos, "pnl_pct": pnl_pct,
                    "numero_operacion_compra": posicion_existente.get("numero_operacion_compra", ""),
                    "numero_operacion_venta": resultado["numero_operacion"],
                    "motivo_entrada": posicion_existente.get("motivo_entrada", ""),
                    "motivo_salida": "manual",
                })
            else:
                print(f"[MANUAL] Venta de {simbolo_opcion} sin posición previa conocida -- "
                      f"no se puede calcular PnL, no se registra histórico.")
            quitar_posicion(pestana_posicion, ticker)
            posiciones_activas.pop(ticker, None)
    ...
    for ticker, posicion in list(posiciones_activas.items()):
        ...
        if dispara_salida:
            ...
            registrar_historico(pestana_historico, {
                ...
                "motivo_entrada": posicion.get("motivo_entrada", ""),
                "motivo_salida": "cruce_ema4_18",
            })
            quitar_posicion(pestana_posicion, ticker)
    ...
    for fila_config in config:
        ...
        fila_posicion = {
            ...
            "numero_operacion_compra": resultado_compra["numero_operacion"],
            "motivo_entrada": "senal_automatica",
        }
        agregar_posicion(pestana_posicion, fila_posicion)
        posiciones_activas[ticker] = fila_posicion  # dict COMPLETO -- para no reabrir en el mismo loop
...
def main():
    ...
    Además, CUALQUIER excepción no manejada (un bug, un cambio de la
    API de IOL, lo que sea) dispara un aviso a Telegram ANTES de
    volver a lanzarla -- sin esto, un crash solo se ve en el log de
    GitHub Actions, y el usuario no se entera hasta que revisa a mano
    (motivo real por el que en algún momento "no llegaban mensajes":
    el bot explotaba antes de llegar a la notificación esperada, y no
    había ningún aviso del crash en sí)."""
    ...
    try:
        ...
        print(detalle)  # queda igual en el log de GitHub para diagnóstico completo
        try:
            tg.notificar_error("CRASH del bot de opciones", f"{type(e).__name__}: {e}\n\n{detalle[-500:]}")
        except Exception:
            pass  # si hasta el aviso de error falla, no hay más nada que hacer acá
        raise  # se re-lanza para que GitHub Actions siga marcando la corrida como fallida
    finally:
