...
def resumen_de_hoy_ya_registrado(sheet) -> bool:
    """Con rutina_cierre corriendo cada 5 min entre 17:00 y 18:00 (para
    tolerar la demora de GitHub), sin este chequeo se generarían hasta
    ~13 filas casi idénticas por día en 'Resumen' -- rompería el
    ...
    if not filas:
        return False
    hoy_str = hoy_argentina().strftime("%d/%m/%Y")
    ultima_fecha = str(filas[-1].get("Fecha Actualización", ""))
    return ultima_fecha.startswith(hoy_str)
...
def rutina_apertura(iol_client):
    """
    Ya NO manda 'conectado'/'desconectado' en cada corrida -- con el
    nuevo esquema de reintento (corre cada 5 min entre 10:25 y 11:30),
    eso generaría ~13 mensajes de puro ruido por sesión. Solo notifica
    cuando efectivamente pasa algo (compra, error) -- mismo criterio que
    ya usaba rutina_monitoreo().
    """
...
def rutina_cierre(iol_client):
    """
    Igual que rutina_apertura: ya no manda 'conectado'/'desconectado' en
    cada corrida (corre cada 5 min entre 17:00 y 18:00) -- solo notifica
    eventos reales (trailing actualizado, cierre, error).
    """
    sheet = sheets.conectar_sheet()
    hoy = hoy_argentina()
    ...
    # --- Registrar una fila de "Resumen" con el estado de hoy (1 SOLA
    #     VEZ por día -- rutina_cierre puede correr hasta 13 veces en la
    #     ventana de reintento 17:00-18:00) ---
    try:
        if not resumen_de_hoy_ya_registrado(sheet):
...
# PUNTO DE ENTRADA
# ============================================================================
# Con un solo disparador cada 5 min (10:25 a 18:00 ART) en vez de 3
# horarios puntuales, se decide acá adentro qué corre según la hora real
# -- así una demora de GitHub en un disparo puntual (documentado, hasta
# 45min-1h45 observado) se compensa solo con el siguiente intento, sin
# depender de acertar el minuto exacto. Las rutinas ya son naturalmente
# "seguras de reintentar" (posición ya abierta -> se salta; ya no hay
# posición -> no hace nada), así que correrlas de más no genera compras
# ni ventas duplicadas.
VENTANA_APERTURA = (dtime(10, 25), dtime(11, 30))
VENTANA_CIERRE = (dtime(17, 0), dtime(18, 0))  # arranca DESPUÉS del
                                                 # cierre real del mercado
                                                 # a propósito -- antes
                                                 # de eso el precio de
                                                 # "hoy" todavía se mueve
                                                 # en vivo, no es el
                                                 # cierre final.
...
if __name__ == "__main__":
    ...
    # Override manual (workflow_dispatch): corre SOLO la rutina pedida,
    # sin importar la hora -- para poder seguir probando cada una suelta
    # como hasta ahora.
    ...
    if modo_manual:
        ...
        # Disparo automático por horario: decide según la hora real.
        hora_actual = datetime.now(TZ_ARGENTINA).time()

        rutina_monitoreo(iol_client)  # siempre -- protege lo que ya está abierto

        if VENTANA_APERTURA[0] <= hora_actual <= VENTANA_APERTURA[1]:
            rutina_apertura(iol_client)

        if VENTANA_CIERRE[0] <= hora_actual <= VENTANA_CIERRE[1]:
            rutina_cierre(iol_client)
