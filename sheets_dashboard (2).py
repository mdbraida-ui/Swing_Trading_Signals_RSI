...
Setup necesario (una sola vez):
  ...
  2. Crear una planilla nueva en Google Sheets con estas 8 hojas (tabs),
  ...
  "Indicadores":
      ...
      Fecha Toque Banda | Días Pendiente | Señal Confirmada | En Cooldown
...
def actualizar_indicadores(sheet, indicadores: dict):
    ...
    try:
        ...
        filas = [["Ticker", "Tipo", "Fecha", "Precio Actual", "RSI14",
                   ...
                   "Señal Pendiente", "Fecha Toque Banda", "Días Pendiente",
    ...
    for r in registros:
        try:
            cola.append({
                "ticker": r["Ticker"],
                "tipo": r.get("Tipo", ""),
                "fecha_confirmacion": r["Fecha Confirmación"],
